...
$sql = "SELECT COUNT(id) FROM nea n where n.category = 5 ";
...
$sql = "SELECT * FROM nea n where n.category = 5  order by ID desc  $limit ";
$query = mysql_query($sql);
...
if($last != 1){
		...
		$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?p=education&c=5'.'&pn='.$previous.'">Προηγούμενο</a> &nbsp; &nbsp; ';
				
				for($i = $pagenum-4; $i < $pagenum; $i++){
			if($i > 0){
		        $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?p=education&c=5&pn='.$i.'">'.$i.'</a> &nbsp; ';
			}
	    }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= ' &nbsp; &nbsp; <a href="'.$_SERVER['PHP_SELF'].'?p=education&c=5&pn='.$next.'">Επόμενο</a> ';
